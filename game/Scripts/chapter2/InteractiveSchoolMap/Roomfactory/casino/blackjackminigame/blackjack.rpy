...
init -100 python:
    ...
    def start_game(bet=10): #fallback because minimum bet is 10
        ...
        global deck, player_hand, round_active, current_bet, dealer_total, game_over, message
        
        
        if store.playerMoney < bet:
            return False
        current_bet = bet
        store.playerMoney -= bet
    ...
    def stand_game():
        global round_active, dealer_total, current_bet, game_over, message
        if game_over or not round_active:
            return

        dealer_total = random.randint(3, 4)
        round_active = False
        ...
        if dealer_total > 21 or player_total > dealer_total:
            message = "You win!"
            store.playerMoney += current_bet * 2
            store.result = "win"
        elif player_total == dealer_total:
            message = "Push."
            store.playerMoney += current_bet
            store.result = "push"
        else:
            message = "Dealer wins."
            store.result = "lose"
